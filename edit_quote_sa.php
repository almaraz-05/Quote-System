<?php
   ...
   try {
     ...
     if ($customer) {
        ...
        $existing_status = $quote['status'] ?? 'open';
     ...
     if (isset($_POST['status_btn'])) {
         ...
         $stmt2 = $pdo->prepare("UPDATE quote SET customer_email = ?, status = ?, quote_price = ? WHERE quote_id = ?");
         $stmt2->execute([$email, $status, $quote_amount, $quote_id]);
...
<html>
   ...
   <body>
    <div class="container">
      ...
      <form action="edit_quote_sa.php" method="POST">
         ...
         <input type="text" id="customerEmail" name="customer_email" required value="<?= htmlspecialchars($email) ?>"><br><br>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    existingNotes.forEach(note => {
        ...
        tr.querySelector('.deleteRowBtn').addEventListener('click', function() {
            tr.remove();
        });
    });
...
document.getElementById('add_line_btn').addEventListener('click', () => {
    ...
    tr.innerHTML = `
        ...
        <td><button type="button" class="deleteBtn" style="width: 60px; padding: 5px">Delete</button></td>
    `;
    document.querySelector('#line_items tbody').appendChild(tr);
    tr.querySelector('.addBtn').addEventListener('click', calculateTotal);
